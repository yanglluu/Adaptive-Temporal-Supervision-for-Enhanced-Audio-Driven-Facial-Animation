...
parser.add_argument('--savepath', type=str,  default='/root/FACIAL-main/audio2face/checkpoint/nodcaw')

# DCAW / 平滑（控制层）——在本脚本中会强制关闭
...
# ⭐ 数据集也固定使用 use_dcaw=False（不再输出时序权重给控制器）
training_set = Facial_Dataset(audio_paths, npz_paths, cvs_paths, use_dcaw=False)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            # =========================
            # 无 DCAW：统一从 batch 里取 x,y，忽略 s,m
            # 兼容 Dataset 返回 2 个 (x,y) 或 4 个 (x,y,s,m)
            # =========================
            if isinstance(batch, (list, tuple)):
                ...
                # 极端情况：DataLoader 只给了一个张量（不太可能）
                x = batch
                raise ValueError("Batch format not supported in no-DCAW mode")

            # ⭐ 无 DCAW：自己造全 1 的时序权重 s,m，作为 TV / jerk 等损失的统一权重
            ...
            # sw（加权）：无 DCAW 时直接用全 1 权重
            sw = s * m
            sw_n  = sw[:, 1:]
            lossg_e  = 20*weighted_mse(yf[:, :, 7:], y[:, :, 7:], sw)
            ...
            # —— 自适应加速度惩罚（pose 前6维）——（此处不再使用 DCAW 权重，只用 sw）
            ...
            if yf.shape[2] >= 6 and yf.shape[1] >= 3:
                ...
                min_tw = min(acc.shape[1], sw.shape[1]-2)
                if min_tw > 0:
                    sw_acc = sw[:, 2:2+min_tw].unsqueeze(-1)   # [B, min_tw, 1]
