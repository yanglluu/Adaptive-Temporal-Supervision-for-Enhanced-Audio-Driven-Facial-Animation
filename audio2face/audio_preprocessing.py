...
#         print("Audio file paths:", audio_list)  
#         tmp_audio = {}
...
tf.compat.v1.disable_v2_behavior()  # 兼容旧版接口

def process_audio(ds_path, audio, fps):
    """使用DeepSpeech处理音频并提取特征"""
    config = {}
    config['deepspeech_graph_fname'] = ds_path
    config['audio_feature_type'] = 'deepspeech'
    config['num_audio_features'] = 29
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='音频预处理：提取DeepSpeech特征并保存为pkl文件')
    parser.add_argument('--audio_path', type=str, help='待处理的音频文件路径')
    parser.add_argument('--output_dir', type=str, default='../examples/audio_preprocessed', help='特征文件输出目录')
    opt = parser.parse_args()
    
    fps = 30  # 帧速率，保持不变
    ds_fname = 'ds_graph/output_graph.pb'  # DeepSpeech模型路径
    
    audio4deepspeech = {}
    subjects = ['single']  # 单文件处理模式的主题名称
    
    # 根据参数决定处理单文件还是多文件
    if opt.audio_path:
        # 单文件处理模式
        audio_list = [opt.audio_path]
        print(f"单文件处理模式：加载音频 {opt.audio_path}")
    else:
        # 原多文件处理模式（默认）
        dataset_path = '../examples/'
        audio_list = []
        for subject in ['audio']:
            audio_list.extend(glob.glob(os.path.join(dataset_path, subject + '/*.wav')))
        print(f"多文件处理模式：加载 {len(audio_list)} 个音频文件")
    ...
    for subject in subjects:
        ...
        for audio_fname in audio_list:
            try:
                # 提取文件名（不含扩展名）
                sentence = os.path.basename(audio_fname).split('.')[0]
                sample_rate, audio = wavfile.read(audio_fname)
                tmp_audio[sentence] = {'audio': audio, 'sample_rate': sample_rate}
                print(f"成功加载音频：{sentence}.wav")
            except Exception as e:
                print(f"加载音频 {audio_fname} 失败：{e}")
        audio4deepspeech[subject] = tmp_audio
    
    # 处理音频并提取特征
    try:
        processed_audio = process_audio(ds_fname, audio4deepspeech, fps)
        print("音频特征提取完成")
    except Exception as e:
        print(f"音频特征提取失败：{e}")
        return
    ...
    if not os.path.exists(out_path):
        try:
            os.makedirs(out_path)
            print(f"创建输出目录：{out_path}")
        except Exception as e:
            print(f"创建输出目录失败：{out_path}，错误：{e}")
            return
    
    # 保存每个句子的音频特征
    saved_files = 0
    for subject in processed_audio.keys():
        for sentence in processed_audio[subject]:
            try:
                out_file = os.path.join(out_path, f"{sentence}.pkl")
                _audio = processed_audio[subject][sentence]['audio']
                pickle.dump(_audio, open(out_file, 'wb'))
                print(f"成功保存特征文件：{out_file}")
                saved_files += 1
            except Exception as e:
                print(f"保存特征文件 {sentence}.pkl 失败：{e}")
    
    print(f"处理完成，共保存 {saved_files} 个特征文件")
