...
parser.add_argument('--savepath', type=str,  default='/root/FACIAL-main/audio2face/checkpoint/no_freq')
...
# ⭐ No Frequency Loss 消融：
# 关闭所有频域相关损失（STFT + banded FM + ultra-short STFT）
opt.use_banded_stft = False
opt.stft_weight = 0.0
opt.stft_weight_pose = 0.0

opt.use_ultra_short_stft = False
opt.ultra_stft_weight = 0.0

opt.use_banded_fm = False
...
def main():
    ...
    # ⭐ 虽然这里仍然实例化，但上面已经把 stft_weight 设为 0，因此实际上不起作用
    ...
    # ⭐ No Frequency：use_banded_fm=False，因此不会创建 banded_fm_loss
    ...
    # ⭐ No Frequency：use_ultra_short_stft=False，因此 ultra_stft_loss=None
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            if real_feats is not None and hasattr(modeldis, 'extract_features'):
                ...
                # ⭐ No Frequency：opt.use_banded_fm=False，因此不会走 banded_fm_loss
            ...
            # AU45 分支（No Frequency 不影响 AU45 分支）
            ...
            if opt.use_au45_branch and au45_branch_loss is not None:
                ...
                try:
                    ...
                    narrow_w = make_au45_narrow_window_local(T_use)
                sw_au = narrow_w.unsqueeze(0).expand(yf.shape[0], -1)
                ...
                # ⭐ No Frequency：opt.use_ultra_short_stft=False，因此不会进这里
            ...
            # ⭐ No Frequency：opt.use_banded_stft=False 且 stft_weight=0，因此这里不会生效
