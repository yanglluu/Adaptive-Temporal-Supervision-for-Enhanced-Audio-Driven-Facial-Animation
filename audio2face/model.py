...
class TfaceGAN(nn.Module):
    ...
    def forward(self, x, y, attn_mask=None, return_feats=False, soft_gate=False, soft_wmin=64, soft_wmax=256):
        """
        Forward pass.
        
        Args:
            x: Input audio features
            y: Input conditioning features
            attn_mask: Optional attention mask (currently unused, reserved for future use)
            return_feats: Whether to return intermediate features
            soft_gate: Whether to use soft gating (currently unused, reserved for future use)
            soft_wmin: Soft gating minimum window (currently unused, reserved for future use)
            soft_wmax: Soft gating maximum window (currently unused, reserved for future use)
            
        Returns:
            If return_feats=True: (output, features)
            Otherwise: output
        """
        x1 = x[:,:,8,:].unsqueeze(1)
        ...
        # Extract features before the last conv layer for feature matching
        feats = None
        if return_feats:
            # Use the output before conv52 as features
            feats = out.permute(0, 2, 1)  # [B, T, C] format
...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input, tmask=None):
        ...
        Args:
            input: Input tensor
            tmask: Optional temporal mask (currently unused, reserved for future use)
        """
        # (64,1,256,150)
